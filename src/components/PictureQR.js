  ...
  const handleDownload = async (e) => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:9090/generatePic',
        { title: formData.title }, // Include only the required field(s)
        {
          headers: {
      ...
      link.href = url;
      link.setAttribute('download', 'generatedQRCode.png');
...
const PictureQR = () => {
  ...
  const handleDownload = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:9090/generatePic', formData, {
